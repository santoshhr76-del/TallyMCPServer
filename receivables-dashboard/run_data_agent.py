"""
Data Agent — fetch outstanding receivables + ledger details from TallyPrime.
Saves:
  output/receivables.json    — verbatim response from get_outstanding_receivables
  output/party_details.json  — enriched party list with address / pincode / state
"""
import sys
import os
import json
...
print(f"Fetching outstanding receivables (as_of_date={AS_OF_DATE}, group='{LEDGER_GROUP}') ...")
...
for i, name in enumerate(party_names, 1):
    print(f"  [{i:>2}/{len(party_names)}] get_ledger: {name}")
