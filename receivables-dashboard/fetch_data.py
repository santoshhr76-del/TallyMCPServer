"""
Fetch outstanding receivables and per-party ledger details from TallyPrime.
...
AS_OF_DATE    = "04-04-2026"   # today + 14 days
...
# Grand total and party count from the response
...
party_names = list({p["party"] for p in party_summary if p.get("party")})
party_names.sort()
print(f"Unique parties to look up: {len(party_names)}")

# Build a quick lookup: party name → outstanding amount
